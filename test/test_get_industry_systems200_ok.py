# coding: utf-8

"""
    EVE Swagger Interface

    An OpenAPI for EVE Online

    The version of the OpenAPI document: 1.19
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from esi_client.models.get_industry_systems200_ok import GetIndustrySystems200Ok

class TestGetIndustrySystems200Ok(unittest.TestCase):
    """GetIndustrySystems200Ok unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> GetIndustrySystems200Ok:
        """Test GetIndustrySystems200Ok
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `GetIndustrySystems200Ok`
        """
        model = GetIndustrySystems200Ok()
        if include_optional:
            return GetIndustrySystems200Ok(
                cost_indices = [
                    esi_client.models.get_industry_systems_cost_indice.get_industry_systems_cost_indice(
                        activity = 'copying', 
                        cost_index = 1.337, )
                    ],
                solar_system_id = 56
            )
        else:
            return GetIndustrySystems200Ok(
                cost_indices = [
                    esi_client.models.get_industry_systems_cost_indice.get_industry_systems_cost_indice(
                        activity = 'copying', 
                        cost_index = 1.337, )
                    ],
                solar_system_id = 56,
        )
        """

    def testGetIndustrySystems200Ok(self):
        """Test GetIndustrySystems200Ok"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
